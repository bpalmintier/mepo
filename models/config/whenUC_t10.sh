#!/bin/bash
#
# Thesis Ch4: When does UC matter: Variations on maintenance tests
#   ERCOT 2007, min 200MW gen, 0% RPS, $0 CO2, Gens from scratch
#   14wk Full Ops w/ Combined Reserves & 80MW min UC integer
#   No B&B priority, No cheat, No cap limit helper
#     -- No Maintenance, Full MILP
#     -- No Maintenance, UCLP: Ignore all UC integer (Inf min) (MILP for investments)
#     -- LP-Maint, Full MILP for UC & Investments
#     -- LP-Maint, UCLP for Investments only
#
...
# ---  ----------  ----- -------------- ---------------------------------
#   1  2012-08-23  13:10  bpalmintier   Adapted from whenUC_bau.sh v2 (whenUC_base v4)
#   2  2012-08-23  13:10  bpalmintier   Added maintenance tests & reduce computation req'ts
...
THIS_FILE_OPT="${ALL_RUN_OPT} --uc_int_unit_min=0.08 --retire=1 --demand=ercot2007_dem_yr_as_14wk.inc --plan_margin=0.05 --rsrv=flex --startup=1 --unit_commit=1 --ramp=1 --min_up_down=1 "

# Note: 25200sec=7hrs
LONG_OPT=" --max_solve_time=25200 "
PAR_OPT=" --par_threads=4 --lp_method=6 --par_mode=-1 --probe=2 "

#======= No Maintenance, Full MILP =======
RUN_CODE="whenUC_t10_y14wk_c0_rps0_flex_rt1_u008_noMaint"
...
#======= No Maintenance, UCLP: Ignore all UC integer (Inf min) (MILP for investments) =======
RUN_CODE="whenUC_t10_y14wk_c0_rps0_flex_rt1_u008_noMaint_uclp"
...
#======= LP-Maint, Full MILP for UC & Investments =======
RUN_CODE="whenUC_t10_y14wk_c0_rps0_flex_rt1_u008_lpMaint"
...
#======= LP-Maint, UCLP for Investments only =======
RUN_CODE="whenUC_t10_y14wk_c0_rps0_flex_rt1_u008_lpMaint_uclp"
...
# Setup run specific OPT
# Note reduced planning margin b/c computing reserves directly
RUN_OPT=" --maint=1 --maint_lp=1 --uc_int_unit_min=Inf "
SHARE_OPT=" ${LONG_OPT} ${THIS_FILE_OPT} "
IO_OPT=" ${COMMON_IO_OPT} -lf=${OUT_DIR}${RUN_CODE}_${GAMS_MODEL}.log -o=${OUT_DIR}${RUN_CODE}_${GAMS_MODEL}.lst --out_prefix=${RUN_CODE}_ --memo=${RUN_CODE}_v${ADVPOWER_REPO_VER}_${DATE_TIME} "

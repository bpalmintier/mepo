...
RUN_CODE="TX07_yr_sep_ud_afine_m01_par4"
...
GAMS_OPTIONS="-errmsg=1 -lf=${OUT_DIR}${RUN_CODE}_${GAMS_MODEL}.log -lo=2 -o=${OUT_DIR}${RUN_CODE}_${GAMS_MODEL}.lst -inputdir=${MODEL_DIR} --out_dir=${OUT_DIR} --out_prefix=${RUN_CODE}_ --memo=${RUN_CODE}_run_time_compare_v${ADVPOWER_REPO_VER}_${DATE_TIME}     --no_nse=1 --par_threads=4 --startup=1 --ramp=1 --out_gen_params=1 --max_solve_time=216000 --sys=ercot2007eGrid_sys.inc --gens=ercot2007eGrid_units_sep.inc --demand=ercot2007_dem_yr.inc --min_up_down=1 --rsrv=separate --mip_gap=0.001 --par_mode=-1 "

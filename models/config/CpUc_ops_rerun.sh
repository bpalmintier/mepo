#!/bin/bash
#
# Thesis Ch4: OPS Re-RUNS When parallel is needed for Ops of non-parallel runs
#  IMPORTANT: configuration list file (*.csv) must contain 3 columns: Output folder, run code, GAMS options
#  Also tolerance is a tad bit less precise (MIP GAP = 0.5%)
#
...
# ---  ----------  ----- -------------- ---------------------------------
#   1  2012-10-05  11:07  bpalmintier   Adapted from CpUc_par_ops.sh v2
...
THIS_FILE_OPT=" ${ALL_RUN_OPT} --adj_rsrv_for_nse=on --demand=ercot2007_dem_yr_as_52wk.inc  --mip_gap=0.005 "

# Note: 82800sec=23hrs
LONG_OPT=" --max_solve_time=82800 --par_threads=${PBS_NP} --lp_method=6 --par_mode=-1 "
...
source ${CONFIG_DIR}/CpUc_opsrun.sh `sed -n "${PBS_ARRAYID} s/[,\t]/ /gp" < ${CONFIG_DIR}/CpUc_ops_rerun_list.csv`

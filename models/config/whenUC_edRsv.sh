...
# ---  ----------  ----- -------------- ---------------------------------
#   1  2012-08-25  01:20  bpalmintier   Adapted from whenUC_simp.sh v6
...
#   4  2012-08-31  17:05  bpalmintier   Adjusted run range (-t) to use adj_rsrv_for_nse for 60% & 80% RPS 
#   5  2012-09-03  08:15  bpalmintier   Adj run range to use derate (to maint) by default for non-uc ops
#   6  2012-09-05  01:30  bpalmintier   ERGH... remove maintenance
#   7  2012-09-06  20:27  bpalmintier   Update range to use offline non-UC reserves by default
...
source ${CONFIG_DIR}/whenUC_run.sh `sed -n "${PBS_ARRAYID} s/[,	]/ /gp" < ${CONFIG_DIR}/whenUC_edRsv_list.csv`

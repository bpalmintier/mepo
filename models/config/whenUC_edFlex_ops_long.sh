#!/bin/bash
#
# Thesis Ch4: EXTRA Time OPS RUNS for When does UC matter Grid ARRAY Base [Manually copy & config]
...
# ---  ----------  ----- -------------- ---------------------------------
#   1  2012-09-21  00:27  bpalmintier   Adapted from whenUC_edFlex_ops.sh v1
...
source ${CONFIG_DIR}/whenUC_opsrun.sh `sed -n "${PBS_ARRAYID} s/[,	]/ /gp" < ${CONFIG_DIR}/whenUC_edFlex_list.csv`

...
COMMON_OPTIONS=" -errmsg=1  -lo=2  -inputdir=${MODEL_DIR} --out_dir=${OUT_DIR} --no_nse=1 --sys=mftest_sys.inc --gens=gens_all_new.inc --rps=0.2 --co2cost=80 --demscale=0.2 --min_gen_size=0.2 --update=../data/TX07_r20_c80_with_wind_rsrv_cap_update.inc "

LONG_OPTS=" --par_threads=8 --lp_method=6 --max_solve_time=111000 --par_mode=-1 --probe=2 "

#======= Full Ops BUT No rsrv only function of wind cap not wind power =======
RUN_CODE="TX07_yr_x0_2_rps20_rv_c_rmp_st_ud_help"
...
#======= Full Ops BUT No rsrv only function of wind cap not power UCLP =======
RUN_CODE="TX07_yr_x0_2_rps20_uclp_rv_c_rmp_st_ud_help"
...
# Setup run specific options
GAMS_OPTIONS=" -lf=${OUT_DIR}${RUN_CODE}_${GAMS_MODEL}.log -o=${OUT_DIR}${RUN_CODE}_${GAMS_MODEL}.lst ${COMMON_OPTIONS} --rsrv=flex --startup=1 --min_up_down=1 --ramp=1 --no_int_uc=1 "

...
#PBS -q xlong
# Adjust the wall time if needed (Note: requires ~150hr for 8760 ops)
...
MATLAB_COMMAND="mfTest_stoc30GWuc8760dp=RunSimsMultiFidTest(''stoc'',[],''UnitCommit'',[],30, true,[],2,''dp'')"
COMMAND_FOR_FILENAME="RunSimsMultiFidTest_02_30GWuc8760dp"
# >>>>>>>>>>>>>>>>>>>>>>>> End Change block <<<<<<<<<<<<<<<<<<<<<<<<<<

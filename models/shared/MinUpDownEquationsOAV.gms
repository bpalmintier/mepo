...
$ontext
----------------------------------------------------
 Ostrango best formulation
  formulation for min up & down time that does not use separate startup or
...
eMinUpTime(B, T, G, S)$( B_SIM(B)
    ...
    pGen(G, 'min_up', S)*vStartup(B, T, G, S)
    =l=
    vUnitCommit(B, T, G, S)
...
eMinDownTime(B, T, G, S)$( B_SIM(B)
                        ...
                        and pGen(G,'gen_size', S) <> 0 ) ..
    pGen(G, 'min_down', S)*vShutDown(B, T, G, S)
    =l=
    pGen(G, 'min_down', S) - 1
    + vUnitCommit(B, T, G, S) 

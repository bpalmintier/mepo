...
$ontext
----------------------------------------------------
Carrion & Arroyo based Unit Commitment Model
----------------------------------------------------
Carrion & Arroyo formulation for unit commitment
  
WARNINGS: 
    -- DOES NOT WORK WITH CLUSTERING
    -- TOTAL FUEL USE AND CO2 EMISSIONS DO NOT INCLUDE STARTUP USAGE
...
---  ----------  ----- -------------- ---------------------------------
  ...
  27 2012-01-28  02:00  bpalmintier   Extracted writeResults
  28 2012-01-28  02:05  bpalmintier   Call UnitCommit for operations model rather than duplicating equations
  ...
  33 2012-03-09  12:45  bpalmintier   Replace -- with mDemandShift for optional loop startup
  34 2012-09-02  17:08  bpalmintier   Replace all $set with $setglobal (to prevent scope errors)
-----------------------------------------------------
$offtext
...
positive variables
   ...
   vPwrOutLimit(D, G, S) "max possible production of unit during time period [GW]"
...
$if set min_up_down $include %shared_dir%MinUpDownEquationsCA
...
*   1x      vPwrOut         GW
*   1x      Demand(dur)     hr
eVarOMCost(S)    .. vVariableOMCost(S)*1e3 =e= sum[(D, G), pGen(G,'c_var_om', S)*vPwrOut(D, G, S)*pDemand(D, 'dur', S)];
...
*
* Units & Scaling:              external            this eq.
*   1/1000x vStartCost          Kusd        to      Musd
...
positive variable
    ...
    ;
equation
    eStartUpCostPerTime(D, G, S)
    ;
    
    eStartupCostPerTime(D, G, S)$G_UC(G) .. vSuCost(D, G, S) =e=
                                        (vUnitCommit(D, G, S) - vUnitCommit(mDemShift(D,1), G, S))
                                            * ( pGen(G, 'c_start_fix', S) 
                                                + sum[(GEN_FUEL_MAP(G, F)), 
                                                        pGen(G, 'fuel_start', S) * pFuel(F, 'cost', S)
                                                     ]
                                              );

    eStartupCost(S)  .. vStartupCost(S)/1e3 =e= 
                            sum[(D,G_UC), vSuCost(D, G_UC, S)];
...
$ifthen not set no_nse
    eNonServedCost(S) .. vNonServedCost(S) =e=
                                sum[(D), vNonServed(D, S)*pPriceNonServed*pDemand(D, 'dur', S)]/1e3;
$endif
...
eInstantFuelByGen(D, G, S)$(pGen(G,'gen_size', S) > 0 and not G_PWL_COST(G)) .. 
...
*   1x  vPwrOut         GW
*   1x  vUnitCommit     integer (no units)
ePiecewiseFuelByGen (D, G, HR_SEG, S)$(PWL_COST_SEG(G, HR_SEG) and pGen(G,'gen_size', S) > 0) ..
...
eFuelUse(F,G,S)$(GEN_FUEL_MAP(G,F)) .. vFuelUse(F,G,S)*1000 =e= sum[(D), 
                                                vInstantFuel(D, G, S)*pDemand(D, 'dur', S)
                                            ];
...
* Units & Scaling:      external        this eq.
*   all in GW
eDemand (D,S)    .. sum[(G), vPwrOut(D, G,S)]
$ifthen not set no_nse
                    + vNonServed(D,S)
$endif
                    =e= pDemand(D,'power',S);
...
*   vPwrOut & gen_size      GW
*   vUnitCommit             integer
ePwrMaxUC (D, G, S)$(G_UC(G) and pGen(G, 'gen_size', S)) .. vUnitCommit(D,G,S) * pGen(G, 'gen_size', S)
                            =g= vPwrOutLimit(D, G, S);

equations
    ePwrBelowPwrLim (D, G, S)
    ePwrLimBelowMax (D, G, S)
    ;
    
    ePwrBelowPwrLim (D, G, S) .. vPwrOut(D, G, S) =l= vPwrOutLimit(D, G, S);
    ePwrLimBelowMax (D, G, S) .. vPwrOutLimit(D, G, S) =l= pGen(G, 'gen_size', S);
...
*   vPwrOut & capacity      GW
*   p_min                   p.u.
ePwrMin (D, G, S) ..   vPwrOut(D, G, S) =g= %capacity_G% * pGen(G,'p_min', S);
...
*   vPwrOut & unit_min      GW
*   vUnitCOmmit             #units
ePwrMinUC (D, G, S)$G_UC(G) .. vPwrOut(D, G, S) =g= vUnitCommit(D,G,S) * pGen(G, 'unit_min',S);
...
*   Demand(dur)     hr
*   pRPS            p.u.
eRPS(S)        ..  sum[(D, G_RPS), vPwrOut(D, G_RPS,S)*pDemand(D, 'dur',S)] =g=
                       pRPS(S)*sum[(G, D), vPwrOut(D,G,S)*pDemand(D, 'dur', S)];
...
* If capacity if fixed, we can use minimum of available power and demand (both parameters)
   eForceRenewables(D, S) .. sum[(G)$G_RPS(G), vPwrOut(D, G, S)] =e= 
...
*   pGenAvail                               p.u.
   eForceRenewables(D, G, S)$G_RPS(G) .. vPwrOut(D, G, S) =e= 
...
eUnitCommit(D,G,S)$G_UC(G)  .. vUnitCommit(D,G,S)
                                    =l=  
...
* G_UC_INT subset. The redundant continuous variable should be removed during (MI)LP pre-solve
   eUnitCommitInteger(D,G,S)$G_UC_INT(G)  .. vUnitCommit(D,G,S) =e= vUcInt(D,G,S);
...
$ifthen set compute_state
*Note: the $subset(setname) format only defines the equation for members of G that are also in G_UC
   eState  (D,G,S)$G_UC(G)  .. vUnitCommit(D,G,S) =e= vUnitCommit(mDemShift(D,1),G,S) + vStartUp(D,G,S) - vShutDown(D,G,S);
$endif
$offtext
...
$ifthen set max_start
   eMaxStart(G,S)$( (pGen(G,'gen_size',S) > 0) and (pGen(G, 'max_start',S) < Inf) ) .. 
      sum[(D), vStartUp(D,G,S)] =l= %capacity_G% / pGen(G,'gen_size',S) * pGen(G, 'max_start',S);
$endif
$offtext
...
*   vUnitCommit   #units
    ...
    eRampUpLimitUC(D,G,S)$(G_UC(G) and G_RAMP(G)) ..
                vPwrOutLimit(D, G, S)
                =l=
                vPwrOut(mDemShift(D,1), G, S)
                + pGen(G, 'ramp_max', S)*pGen(G, 'gen_size', S) * vUnitCommit(mDemShift(D,1),G,S)
                + max(pGen(G, 'unit_min',S), pGen(G, 'ramp_max',S)*pGen(G, 'gen_size',S)
                     ) * (vUnitCommit(D,G,S) - vUnitCommit(mDemShift(D,1),G,S))
* Note: this final term required to prevent negative power output when unit is first turned off
* It offsets the above term which is negative when first turned off, but goes to zero to not
* influence the decision when the unit is running
                + pGen(G, 'gen_size', S) * (1 - vUnitCommit(D,G,S))
                ;
...
*   pGenAvail               p.u.
    ...
    eRampUpLimit(D,G,S)$(G_RAMP(G) and not G_UC(G)) ..
...
*   pGenAvail               p.u.
    ...
    eRampDownLimit(D,G,S)$(G_RAMP(G) and not G_UC(G)) ..
                    vPwrOut(mDemShift(D,1), G, S) - vPwrOut(D, G, S)
                    =l=
                    pGen(G, 'ramp_max', S) * ( %capacity_G% *pGenAvail(mDemShift(D,1), G, S) - vPwrOut(D--1, G, S));
$endif.ramp_eq
...
$ifthen set unit_commit
    vUcInt.up(D, G_UC, S) = pMaxNumPlants(G_UC, S);
$endif
